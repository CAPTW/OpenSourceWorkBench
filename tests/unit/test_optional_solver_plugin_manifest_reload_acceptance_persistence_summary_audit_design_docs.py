...
def test_design_doc_exists_and_records_design_only_non_actions() -> None:
    ...
    required = (
        "design-only",
        "no summary audit implementation",
        "no CLI source edits",
        "no GUI source edits",
        "no source edits",
...
def test_design_doc_defines_future_summary_audit_contract() -> None:
    text = _read(DOC)
    required = (
...
def test_meta_docs_record_adr_guardrails_risk_checklist_and_validation() -> None:
    ...
    _assert_any(decision_log, "ADR-0169")
    _assert_any(
        decision_log,
        "Optional Solver Plugin Manifest Reload Acceptance Persistence "
        "Summary Audit Is Non-Authoritative",
    )
    _assert_any(guardrails, "reload acceptance persistence summary audit design")
    _assert_any(guardrails, "summary audit implementation")
    _assert_any(risk_register, "reload acceptance persistence summary audit")
    _assert_any(risk_register, "summary audit overreach")
    _assert_any(validation_matrix, "reload acceptance persistence summary audit")
    _assert_any(validation_matrix, "docs/test design evidence only")
    _assert_any(
        release_checklist,
        "reload acceptance persistence summary audit remains design-only",
    )
    _assert_any(release_checklist, "does not implement summary audit behavior")
    _assert_any(
        changelog,
        "Optional Solver Plugin Manifest Reload Acceptance Persistence "
        "Summary Audit Design",
    )
