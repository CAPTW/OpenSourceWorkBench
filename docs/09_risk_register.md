...
| Version/tag procedure unclear | A release tag could be created with mismatched package metadata or unresolved blockers | Keep `docs/13_license_and_version_plan.md` as the tag procedure source and require a dedicated tag prompt | Do not create tags in planning prompts; leave release state as blocked until P1 items clear |
| Third-party notices incomplete | Optional dependencies and plugin ecosystems may have redistribution obligations that are not captured in source docs | Review notices before source/wheel release and before any binary distribution | Ship source-only documentation path and avoid bundled external solver binaries |
