...
| Version/tag gate bypass | A release tag could be created before QA, notices, or final checklist evidence are reviewed | Keep `docs/13_license_and_version_plan.md` as the tag procedure source and require a dedicated tag prompt | Do not push or announce; delete only local tags in a later prompt if maintainers reject an unpushed tag |
