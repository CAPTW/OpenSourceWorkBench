...
| FEASpec IR design | [FEASpec IR design](experimental/feaspec_ir_design.md) and `tests/unit/test_feaspec_ir_docs.py` | FEASpec is documented as design-only, experimental, and not implemented. The IR contract records candidate versus approved spec, explicit units, GeometryGraph, materials/sections, BC/load targets, evidence/confidence, diagnostics, validation states, and CalculiX-first compatibility without automatic solver execution, VLM API integration, mandatory Abaqus, certification, or native commercial CAD import. | design evidence |
