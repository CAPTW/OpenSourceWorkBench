...
| FEASpec Python models | [FEASpec Python models](experimental/feaspec_python_models.md), `src/osw/experimental/feaspec/`, `tests/unit/test_feaspec_models.py`, and `tests/unit/test_feaspec_model_guardrails.py` | Experimental dataclass models and JSON helpers load, basic-check, and serialize existing FEASpec examples and benchmark seeds. Checks are structural only: required fields, explicit units, geometry IDs/references, BC/load targets, and approved human-review state. No full validator, ProjectSchema bridge, solver adapter, VLM API, solver execution, mandatory Abaqus, or certification claim is added. | automated model evidence |
