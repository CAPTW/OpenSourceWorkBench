...
- turns prepared-machine validation command implementation into dependency
  installation, solver installation, solver execution without a separate
  explicit approval gate, arbitrary user project workloads, live network/
  provider/OAuth/MCP behavior, ProjectSchema mutation or evidence creation,
  issue/release/tag/asset mutation, validation success/failure overclaim,
  issue closure claim, bundled solver support claim, certification claim,
  default evidence write, checked-in runtime evidence, release asset evidence,
  or hidden evidence output;
...
- turns v0.1.5-rc1 metadata alignment into tag creation, release
  ...
  exist;
- turns the OpenFOAM template v12 compatibility fix design into OpenFOAM
  source/template implementation, golden fixture updates, solver execution, live
  validation, issue mutation, release/tag/asset mutation, ProjectSchema mutation,
  version bump, certification or production-readiness claims, bundled-solver
  support claims, or native-Windows validation claims without a separate
  implementation/validation gate;
- turns the OpenFOAM template v12 compatibility fix implementation into live
  solver execution, OpenFOAM runtime validation, unit tests that require an
  installed OpenFOAM, a blanket `transportProperties`-to-`physicalProperties`
  rename that drops legacy/ESI support, issue closure/comment/mutation,
  release/tag/asset mutation, ProjectSchema mutation or evidence, version bump,
  dependency/solver installation, or certification, production-readiness,
  bundled-solver, or native-Windows validation claims;
- turns the OpenFOAM v12 `pFinal` fix design into `fvSolution` source/template
  implementation, golden fixture updates, adding `pFinal` to SIMPLE/`simpleFoam`
  cases, solver execution, live validation, issue mutation, release/tag/asset
