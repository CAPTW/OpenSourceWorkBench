...
| Current patch candidate `0.1.3rc1` / `v0.1.3-rc1` | `PASS_WITH_WARNINGS` | OSW-RELEASE-003 aligns source metadata and release docs to `0.1.3rc1`. The planned local annotated tag is `v0.1.3-rc1`, but this metadata gate creates no tag, push, package artifact, or public announcement. |
| Public tag push | `P1 BLOCKED` | Do not push `v0.1.0-rc1`, `v0.1.0-rc2`, `v0.1.0-rc3`, historical `v0.1.0`, local `v0.1.1-rc1`, local `v0.1.1`, local `v0.1.2-rc1`, local `v0.1.2`, or any later `v0.1.3-rc1` candidate unless a later explicit maintainer push/version gate approves the exact current release path. |
...
| REL-003-P1-001 | P1 | Patch v0.1.3rc1 metadata gate | `PASS_WITH_WARNINGS` | OSW-RELEASE-003 aligns current package metadata to `0.1.3rc1`, updates release docs and metadata QA defaults, and preserves historical tags through `v0.1.2`. | Create `v0.1.3-rc1` only in a later explicit local tag gate after post-commit QA and clean staged-artifact checks. |
