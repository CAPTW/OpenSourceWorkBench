...
| VFEA experimental scope | `PASS` | [VFEA experimental scope definition](roadmap/vfea_experimental_scope.md) records issue `#17` as planning-only and experimental, with FEASpec candidate/validator/human-review/benchmark requirements, CalculiX-first planning, optional/non-default Abaqus export planning only, no automatic unreviewed solver execution, no topology optimization implementation, and no certification claim. |
...
| Next feature gate | `PENDING` | After VFEA scope definition, use `OSW-EXP-001A_VFEA_SCOPE_CLOSURE` if maintainers accept the scope, or `OSW-EXP-002_FEASPEC_IR_DESIGN` for the next planning-only design step. Live optional validation issues `#6`-`#11` remain deferred to suitable environments. |
