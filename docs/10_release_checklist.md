...
| --- | --- | --- |
| Release gate result | `PENDING` | OSW-AUTO-070 prepares patch RC package metadata `0.1.2rc1` and the local annotated `v0.1.2-rc1` tag gate after OSW-AUTO-068 verified the GUI workflow fix with no P0/P1 blockers. Historical local `v0.1.1` remains preserved and must not be published as current. |
...
| Release version plan | `PASS` | [License and version plan](13_license_and_version_plan.md) defines current package metadata `0.1.2rc1`, historical local tags `v0.1.0-rc1`, `v0.1.0-rc2`, `v0.1.0-rc3`, `v0.1.0`, `v0.1.1-rc1`, and `v0.1.1`, and the selected next patch path `0.1.2rc1` / `v0.1.2-rc1` -> `0.1.2` / `v0.1.2`. |
...
| Next patch candidate `0.1.2rc1` / `v0.1.2-rc1` | `PENDING` | OSW-AUTO-070 prepares package metadata `0.1.2rc1`, release metadata checker support, source-install evidence, compact GUI workflow evidence, and the local annotated `v0.1.2-rc1` tag gate. The tag may be created only after post-merge checks pass; final `v0.1.2` remains blocked. |
...
| REL-070-P1-001 | P1 | Patch v0.1.2 RC1 local tag gate | `PENDING` | OSW-AUTO-070 prepares the package version `0.1.2rc1`, validates historical `v0.1.0` and `v0.1.1` final tags plus prior RC tags, runs fresh source-install and compact GUI workflow validation, and may create local annotated `v0.1.2-rc1` only after all post-merge checks pass. | Keep `v0.1.2` final and public push blocked until later explicit gates. |
...
| Item | Status | Evidence / notes |
| --- | --- | --- |
| `pyproject.toml` metadata, package discovery, and `requires-python >=3.11` are current. | `PASS` | Package version is `0.1.2rc1`; Python requirement is `>=3.11`; license metadata is `GPL-3.0-or-later`. |
...
| `python -m osw.cli --version` reports the intended version. | `PASS` | Expected command output after OSW-AUTO-070 metadata alignment: `osw 0.1.2rc1`. |
...
| `python -m osw.cli --version` reports intended version. | `PASS` | Expected output after OSW-AUTO-062 metadata alignment: `osw 0.1.1`. |
