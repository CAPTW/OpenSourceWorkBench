...
| --- | --- | --- |
| Release gate result | `PASS` | OSW-AUTO-077 published the final v0.1.2 source release by pushing `develop` and annotated `v0.1.2` to GitHub after OSW-AUTO-072/073/075/076E verified metadata, source-install smoke, and publish-clone readiness. |
...
| Release version plan | `PASS` | [License and version plan](13_license_and_version_plan.md) defines current package metadata `0.1.2`, historical local tags `v0.1.0-rc1`, `v0.1.0-rc2`, `v0.1.0-rc3`, `v0.1.0`, `v0.1.1-rc1`, `v0.1.1`, and `v0.1.2-rc1`, and records the published GitHub `v0.1.2` source tag. |
...
| Final `v0.1.2` metadata prep | `PASS` | OSW-AUTO-072 prepares final package metadata `0.1.2`, final release notes, release metadata checks, source-install evidence, compact GUI workflow evidence, and final tag gate routing without creating `v0.1.2` or pushing. |
...
| Item | Status | Evidence / notes |
| --- | --- | --- |
| `pyproject.toml` metadata, package discovery, and `requires-python >=3.11` are current. | `PASS` | Package version is `0.1.2`; Python requirement is `>=3.11`; license metadata is `GPL-3.0-or-later`. |
...
| `python -m osw.cli --version` reports the intended version. | `PASS` | Expected command output after OSW-AUTO-072 metadata alignment: `osw 0.1.2`. |
...
| `python -m osw.cli --version` reports intended version. | `PASS` | Expected output after OSW-AUTO-072 metadata alignment: `osw 0.1.2`. |
| `python -m osw.cli doctor` reports environment status. | `PASS` | Doctor command passed with optional dependency diagnostics. |
| `python tools/qa/check_release_metadata.py` passes. | `PASS` | Verifies `LICENSE`, pyproject license/version metadata, README License section, release notes, third-party notices draft, expected historical rc/final tags, unexpected release-tag absence, and annotated tag targets when expected tags are supplied. |
