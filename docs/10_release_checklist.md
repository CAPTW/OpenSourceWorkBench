...
| Next feature gate | `PENDING` | After VFEA scope closure, use `OSW-EXP-002_FEASPEC_IR_DESIGN` for the next planning-only design step, run an `OSW-VALID` live optional validation gate on a suitable machine, or prepare the next `v0.1.3rc2` release-planning boundary. Live optional validation issues `#6`-`#11` remain deferred to suitable environments. |
